...
import { X, Minus, Plus, Trash2, Loader2, ChevronDown, ChevronUp } from "lucide-react";
import { useCart } from "../context/CartContext";
import { formatPrice, extraItems } from "../data/menu";
        ...
        <div className="flex h-full flex-col">
          ...
          {lines.length > 0 && (
            <div className="border-t border-charcoal/10 px-6 py-5">
              <div className="mb-4 rounded-2xl bg-charcoal/5">
                <button
                ...
                {extrasOpen && (
                  <ul className="max-h-56 space-y-1 overflow-y-auto px-4 pb-4">
                    {extraItems.map((extra) => (
                      <li key={extra.id} className="flex items-center justify-between gap-3 text-sm">
                        <span>{extra.name}</span>
                        <button
                          onClick={() => addItem(extra)}
                          className="flex shrink-0 items-center gap-1 rounded-full bg-white px-3 py-1 font-display font-bold uppercase text-charcoal shadow-sm hover:bg-flame hover:text-cream"
                        >
